...
class DB:
    ...
    def insert(self, process_id, start_time, file_name=None, file_path=None,
        ...
        query = f'''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?);
        '''
        self._connection.execute(query, (process_id, file_name, file_path, description, start_time, end_time, percentage))
        self._connection.commit()
	      ######################################## YOUR CODE HERE ##################################################
    ...
    def update_percentage(self, process_id, percentage):
        ...
        query = f'''
            ...
            WHERE process_id = ?;
        '''
        self._connection.execute(query, (percentage, process_id))
        self._connection.commit()
######################################## YOUR CODE HERE ##################################################